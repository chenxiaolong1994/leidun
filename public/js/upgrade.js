...
function exec(title, url, progressBar, startProgress, endProgress) {
    ...
    $.ajax(prepareUrl(url), {
        ...
        progressBar.error(url + '时，发生了错误'+'<br>'+jqXHR.responseText);
...
$("#finish-update-btn").click(function() {
	var domain=$("#nextversion1").val();
	var version=$("#domain").val();
	$.ajax({
		 type: "get",
		 async: false,
		 url: "http://www.yxtcmf.com/index.php?g=api&m=Regcheck&a=addupdate",
		 dataType: "jsonp",
		 data : {
				host : domain,
				version : version
